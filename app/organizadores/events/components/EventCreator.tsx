...
const createTicketDraft = (): TicketTierDraft => ({
...
const EventCreator: FC = () => {
  ...
  const [ticketDrafts, setTicketDrafts] = useState<TicketTierDraft[]>(() => [createTicketDraft()]);
  ...
  const handleTicketDraftChange = (
    tierId: string,
    field: 'name' | 'currency' | 'price' | 'available',
    value: string
  ): void => {
  ...
  const addTicketTier = (): void => {
    setTicketDrafts((prev) => [...prev, createTicketDraft()]);
  };

  const removeTicketTier = (tierId: string): void => {
    setTicketDrafts((prev) => {
      if (prev.length === 1) {
        return prev;
      }
  ...
  const resetForm = (): void => {
    ...
    setTicketDrafts([createTicketDraft()]);
  };

  const toPayload = (status: EventStatus): AdminEventPayload => {
    const ticketTiers = ticketDrafts
  ...
  const validateForm = (): string | null => {
    ...
    const validTicketDrafts = ticketDrafts.filter((tier) => tier.name.trim() !== '');

    if (validTicketDrafts.length === 0) {
      return 'Agrega al menos un tipo de ticket con nombre identificable.';
    }

    const hasPositiveTickets = validTicketDrafts.some(
      (tier) => Number(tier.price) > 0 && Number(tier.available) > 0
    );

    if (!hasPositiveTickets) {
      return 'Configura al menos un tipo de ticket con precio y stock disponible.';
    }
  ...
  const totalPlannedCapacity = useMemo(() => {
    return previewTicketSummary.reduce((acc, tier) => acc + tier.availableNumber, 0);
  }, [previewTicketSummary]);
    ...
    <div className='admin-event-grid'>
      <div>
        ...
        <div className='admin-card'>
          ...
          <p className='gray'>
            Define los precios y capacidad disponible para cada tipo de ticket.
          </p>
            ...
            {ticketDrafts.map((tier) => (
              <div key={tier.id} className='admin-ticket-card'>
                <div className='admin-ticket-card__header'>
                  <div className='admin-ticket-card__field admin-ticket-card__field--grow'>
                    <label htmlFor={`name-${tier.id}`}>Nombre del ticket</label>
                    <input
                      id={`name-${tier.id}`}
                      type='text'
                      name='name'
                      className='input-text admin-input'
                      value={tier.name}
                      placeholder='Ej: Backstage, General, Experiencia grupal'
                      onChange={(event) => {
                        handleTicketDraftChange(tier.id, 'name', event.target.value);
                      }}
                    />
                  </div>
                  <button
                    type='button'
                    className='admin-ticket-card__remove'
                    onClick={() => {
                      removeTicketTier(tier.id);
                    }}
                    aria-label='Eliminar tipo de ticket'
                    disabled={ticketDrafts.length === 1}
                  >
                    <span className='material-symbols-outlined'>delete</span>
                  </button>
                </div>
                <div className='admin-ticket-card__field admin-ticket-card__field--inline'>
                  <label htmlFor={`currency-${tier.id}`}>Moneda</label>
                  <input
                    id={`currency-${tier.id}`}
                    type='text'
                    name='currency'
          ...
          <div className='admin-card__footer admin-card__footer--inline'>
            <button
              type='button'
          ...
          <div className='admin-preview-card'>
            ...
            <div className='admin-preview-card__footer'>
              ...
              <div>
                ...
                Compra libre habilitada · Stock planificado:{' '}
                <strong>
